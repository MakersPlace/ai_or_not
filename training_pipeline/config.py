...
SM_LOCAL_PATH_PREFIX = "/opt/ml"
IS_SAGEMAKER = Path(SM_LOCAL_PATH_PREFIX).exists()
SM_ROLE = os.environ.get("SM_ROLE")
...
TRAIN_DIRECTORIES = [
    ...
    ("DIRE/train/imagenet/real_metadata.csv", 0, 0.05),  # 40K
    ("DIRE/train/celebahq/real_metadata.csv", 0, 0.2),  # 28K
    ("DIRE/train/lsun_bedroom/real_metadata.csv", 0, 0.1),  # 40K
    ("cifake/train/REAL_metadata.csv", 0, 0.1),  # 32x32 50k
    ("gans/train_0_real_metadata.csv", 0, 0.01),  # 360K Pro GAN
    ...
    ("gans/train_1_fake_metadata.csv", 1, 0.1),  # 256x256 360K Pro GAN
    ...
    ("DIRE/train/imagenet/adm_metadata.csv", 2, 0.1),  # 256x256 40K
    ("DIRE/train/lsun_bedroom/adm_metadata.csv", 2, 0.1),  # 256x256 40K
    ("DIRE/train/lsun_bedroom/iddpm_metadata.csv", 2, 0.1),  # 256x256 40K
    ("DIRE/train/lsun_bedroom/pndm_metadata.csv", 2, 0.1),  # 256x256 40K
    # SD V1_5
...
def get_path_config(pipeline_name):
    ...
    path_config = {
        ...
        # S3 CHECKPOINTS PATH
        "S3_CHECKPOINTS_PATH": f"{s3_training_output_path}/checkpoints",
        "S3_MODEL_OUTPUT_PATH": f"{s3_training_output_path}/model",
    }

    if IS_SAGEMAKER:
        # --------------------- REMOTE DATA GENERATION STEP ---------------------- #
        path_config["INPUT_PATH"] = f"{SM_LOCAL_PATH_PREFIX}/processing/input"
        ...
        path_config["MODEL_DIR"] = f"{SM_LOCAL_PATH_PREFIX}/model"
        path_config["CHECKPOINTS_PATH"] = f"{SM_LOCAL_PATH_PREFIX}/checkpoints"
    else:
        local_pipeline_path = f"{LOCAL_PATH_PREFIX}/{pipeline_name}"
        local_data_generation_output_path = f"{local_pipeline_path}/{data_generation_output_prefix}"
        local_training_output_path = f"{local_pipeline_path}/{training_output_prefix}"
        # --------------------- LOCAL DATA GENERATION STEP ---------------------- #
        path_config["INPUT_PATH"] = local_pipeline_path
        path_config["CACHE_PATH"] = f"{local_pipeline_path}/cache"
